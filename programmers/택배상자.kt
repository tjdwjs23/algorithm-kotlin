...
class Solution {
    fun solution(order: IntArray): Int {
        ...
        var orderIndex = 0 // order 배열을 순회하기 위한 인덱스

        for (i in 1..order.size) {
            stack.addLast(i) // 스택에 순서대로 추가
            // 스택의 마지막 요소와 현재 order 요소가 일치하는 경우
            while (stack.isNotEmpty() && orderIndex < order.size && stack.last() == order[orderIndex]) {
                stack.removeLast() // 스택에서 제거
                orderIndex++ // 주문 처리, 다음 주문으로
                answer++ // 처리된 주문 수 증가
            }
        }
