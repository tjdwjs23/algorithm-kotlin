...
fun main() = System.`in`.bufferedReader().run {
    ...
    // 선수의 수를 저장할 Map을 초기화합니다.
    val players = mutableMapOf<Char, Int>()

    repeat(n) {
        // 선수의 성을 입력받습니다.
        val name = readLine()
        // 성의 첫 글자를 추출하여 Map에 저장합니다.
        val firstChar = name[0]
        // 선수의 수를 1 증가시킵니다.
        players[firstChar] = players.getOrDefault(firstChar, 0) + 1
    }

    // 선수의 수가 5명 이상인 성의 첫 글자를 사전순으로 출력합니다.
    val result = players.filter { it.value >= 5 }.keys.sorted().joinToString("")
    // 선수의 수가 5명 이상인 성의 첫 글자가 없는 경우 "PREDAJA"를 출력합니다.
    println(if (result.isEmpty()) "PREDAJA" else result)
