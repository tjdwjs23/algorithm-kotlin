...
fun main() = System.`in`.bufferedReader().run {
    val n = readLine().toInt() // 컴퓨터의 수
    val m = readLine().toInt() // 연결 쌍의 수
    val visited = BooleanArray(n + 1)
    val graph = List(n + 1) { mutableListOf<Int>() }

    repeat(m) { // 연결 쌍의 수만큼 반복
    ...
    tailrec fun dfs(v: Int) {
        if (visited[v]) return
        visited[v] = true
        graph[v].forEach { dfs(it) }
    }

    dfs(1) // 1번 컴퓨터에서 시작하여 DFS 실행

    // 1번 컴퓨터를 제외하고 방문한 컴퓨터의 수를 출력
