...
fun main() = System.`in`.bufferedReader().run {
    // n: 노래의 수, l: 노래의 길이, d: 전화벨 울리는 주기
    var (n, l, d) = readLine().split(" ").map { it.toInt() }
    var t = 0
    // 노래의 길이 + 조용한 구간의 길이가 전화벨 울리는 주기보다 작을 때까지 반복합니다.
    while (t < n * (l + 5)) {
        // 노래의 길이 + 조용한 구간의 길이가 전화벨 울리는 주기보다 작으면 반복을 종료합니다.
        if(t % (l + 5) >= l) break
        t = t + d
    }
    println(t)
