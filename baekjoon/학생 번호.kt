...
fun main() = System.`in`.bufferedReader().run {
    ...
    val students = Array(N) { readLine() }
    var k = 0

    // 학생들의 번호를 뒤에서부터 k자리만 추려서 남겨 놓았을 때 모든 학생들의 학생 번호를 서로 다르게 만들 수 있는 최소의 k를 구합니다.
    while (true) {
        val set = mutableSetOf<String>()
        // 학생들의 번호를 뒤에서부터 k자리만 추려서 남겨 놓습니다.
        val filtered = students.map { it.substring(it.length - k) }
        // 중복을 제거한 후, 중복이 없다면 모든 학생들의 학생 번호를 서로 다르게 만들 수 있습니다.
        if (filtered.all { set.add(it) }) break
        k++
    }
    println(k)
}